...
public final class MathUtils {
    ...
    private static final SparseIntArray sIntArray = new SparseIntArray();
     ...
     * @return
     */
    public static long recursive(int number, int range) {
        if (number == 0 || number == 1)
            return number;
        if (range == 1)
            return number;
        return number * recursive(number - 1, --range);
     ...
     * @return
     */
    public static int[] randomIn(int max, int n) {
        sIntArray.clear();
        for (int i = 0; i < max; i++)
            sIntArray.append(i, i);
        int[] array = new int[n];
        int index;
        for (int i = 0; sIntArray.size() > 0 && i < n; i++) {
            index = random(sIntArray.size());
            array[i] = sIntArray.valueAt(index);
            sIntArray.removeAt(index);
        }
        return array;
    }
}